...
def main():
    #defining positions of the corners to build the room
    corners = [
        (0, 0), (5.5, 0), (5.1, 0), (6.21, 4.04)
    ]    

    mat_wall   = get_material("wall")
    carpet = get_material("carpet")
    mat_panel  = get_material("acoustic_foam")
    ...
    histogram=hist,
    src_xyz=[1.36, 3.76, 1.62],
    rec_xyz=[2.03, 0.5, 1.62],
    hrtf=hrtf,
    fs=fs,
    duration=2.0,
    interference=True)
    ...
    np.savez(r"C:\Masters\Hybrid\hybridsim\scenario1\results\rir_scenario1_ism5_200000_S2R2.npz", 
             rir_total = rir_total, 
             rir_bands = rir_bands,
             fs = fs,
             brir_l = brir_l,
