...
from rayroom.room.objects import Furniture, _create_box_vertices_faces
...
def main():
    ...
    room = Room.create_shoebox([5.64, 6.82, 4.32], materials=mats)
    # 2. Add Objectss
    # Source at (1, 1, 1.5)
    source    = Source("Speaker", [1.04, 2.5, 1.62], power=1.0)
    room.add_source(source)


    # Receiver (Microphone) at (4, 3, 1.5)
    receiver1 = Receiver("persona", [4.50, 2.50, 1.62], radius=0.09)
    room.add_receiver(receiver1)
    
    wall_thickness = 0.20
    wall_height = 2.16
    wall_width = 4.10
    verts, faces = _create_box_vertices_faces(
        dimensions = [wall_thickness, wall_width, wall_height],
        center_bottom_pos = [0,0,0]
    )

    partial_wall = Furniture(
        name = "PartialWall",
        position = [2.82, 2.05, 0.0],
        vertices = verts, 
        faces = faces,
        material = get_material("drywall")
    )

    room.add_furniture(partial_wall)
    ...
    np.savez(r"C:\Masters\Hybrid\RayroomProject\examples\initial_testing\wall.npz", 
             rir_total = rir_total, 
             rir_bands = rir_bands,
             fs = fs,
             brir_l = brir_l,
    ...
    direct_ism = [h for h in hist_sorted if h[2] == True][0]   # is_ism=True
    direct_rt  = [h for h in hist_sorted if h[2] == False][0]  # is_ism=False, earliest

    print(f"ISM direct amplitude: {np.mean(np.abs(direct_ism[1])):.6e}")
    print(f"RT  direct amplitude: {np.mean(np.abs(direct_rt[1])):.6e}")
    print(f"Ratio: {np.mean(np.abs(direct_ism[1])) / np.mean(np.abs(direct_rt[1])):.4f}")
